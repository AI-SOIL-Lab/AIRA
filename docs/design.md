...
|-------|---------|---------|-----------|
| `ingest` | 写入 | 高频，按需 | 统一的知识库录入入口，包含 CLI tool 用于文件转换录入 |
| `aminer-free-search` | 读取 | 中频，按需 | AMiner 免费接口，学术实体轻量初筛与发现 |
| `aminer-data-search` | 读取 | 低频，按需 | AMiner 全量接口，深度学术分析与工作流 |
| `aminer-daily-paper` | 读取 | 低频，按需 | 适合追踪最新的研究进展 |
| `research` | 读取+推理 | 高频，按需 | 基于知识库回答任何问题 |
| `health` | 检查+修复 | 低频，定期 | 检查知识库健康程度并修复 |

**统一入口设计：**
- `ingest` skill 是唯一对外暴露的入口，统一处理所有场景
- 用户指定文件/URL → `ingest` 调用 `aira-ingest` CLI tool 录入 → git diff → digest → index
- 用户没有指定文件 → `ingest` 直接 git diff → digest → index（处理用户手动录入的文件）

**学术搜索路由：**
- 学术查询优先路由到 AMiner skill（论文、学者、机构、期刊、专利等）
- 先用 `aminer-free-search` 轻量初筛，需要深度分析时升级到 `aminer-data-search`追踪最新研究进展时使用 `aminer-daily-paper`。
- 与 `research` 的区分：查询外部学术信息 → AMiner；基于知识库分析 → research
...
```
用户：把这篇论文加进来 / 处理新文件 / 保存这个 URL
  ↓
Agent（ingest skill）：
  1. 判断用户意图：是否指定了文件/URL？
     - 是 → Step 2a
     - 否 → Step 2b

  Step 2a：调用 aira-ingest CLI tool 录入
    a. 解析用户输入的内容（PDF/URL/文本）
    b. 选择转换工具（mineru）
    c. 转换为 Markdown
    d. 写入 raw/xxx.md

  Step 2b：Git 检查
    a. git diff HEAD -- vault/raw/ 发现所有新文件
    （不管是 tool 录入的还是用户手动录入的，都在这一步发现）
